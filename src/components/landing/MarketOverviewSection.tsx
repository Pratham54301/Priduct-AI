...
const initialMarketData = [
  ...
  { name: 'HDFC Bank', value: '₹1,710.80', change: '+12.10 (0.71%)', changeType: 'positive' as const, marketType: 'Stock', icon: Briefcase },
  { name: 'Infosys (INFY)', value: '₹1,525.50', change: '+10.10 (0.66%)', changeType: 'positive' as const, marketType: 'Stock', icon: Briefcase },
  { name: 'ICICI Bank', value: '₹1,125.40', change: '-5.80 (0.51%)', changeType: 'negative' as const, marketType: 'Stock', icon: Briefcase },
  ...
  { name: 'XRP (XRP)', value: '$0.475', change: '-0.012 (2.46%)', changeType: 'negative' as const, marketType: 'Crypto', icon: Bitcoin },
  ...
  { name: 'AUD/USD', value: '$0.6650', change: '-0.0015 (0.22%)', changeType: 'negative' as const, marketType: 'Currency', icon: Landmark },
...
export function MarketOverviewSection() {
    ...
    const interval = setInterval(() => {
      setMarketData(currentData =>
        currentData.map(item => {
          // 10% chance to update each item to simulate live data
          if (Math.random() < 0.1) {
            const valueNum = parseFloat(item.value.replace(/[^0-9.]/g, ''));
            const randomFactor = (Math.random() - 0.5) * 0.005; // tiny fluctuation
            ...
            return {
              ...item,
              value: (item.marketType === 'Currency' ? newValue.toFixed(4) : newValue.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })),
  ...
  return (
    <section id="features" className="py-16 md:py-24 bg-card">
      ...
      <div className="mt-8 w-full overflow-hidden relative [mask-image:linear-gradient(to_right,transparent,white_5%,white_95%,transparent)]">
        <div className="flex w-max animate-marquee-slow hover:[animation-play-state:paused]">
          {filteredAssets.map((item, index) => (
            <div key={`${item.name}-${index}-a`} className="px-3">
              <MarketCard {...item} />
            </div>
          ))}
          {/* Duplicate for seamless loop */}
          {filteredAssets.map((item, index) => (
             <div key={`${item.name}-${index}-b`} className="px-3">
              <MarketCard {...item} />
            </div>
          ))}
        </div>
